...
if( is_valid_user($taikhoan) && is_valid_traloi($traloi) && is_valid_email($email)){
	if($pas == $pas2){
	...
	if ($num_row==0)
	{
		...
		//Added by caovietquoc for Account_sub_info Table (Email)
		$sql = "INSERT INTO Account_sub_info(cAccName,cPassWord,cPhone,cEmail) VALUES ('$taikhoan','$matkhau','$fone','$email')";
		mssql_query($sql,$ketnoi)or die ("Đăng ký thông tin không thành công");
		
		mssql_close($ketnoi);
		echo "<script language=Javascript1.1>alert(\"Bạn đã đăng ký thành công. Chuyển đến trang đăng nhập...\");</script>";
		echo("<script language='JavaScript'>window.top.location='../dangnhap';</script>"); 
	}
	else
	{
	...
	{
		echo "<script language=Javascript1.1>alert(\"Mật khẩu không trùng nhau. Chuyển đến trang đăng ký...\");</script>";
		echo("<script language='JavaScript'>window.top.location='index.php';</script>"); 
