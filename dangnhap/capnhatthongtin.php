...
$query = mssql_query("select * from Account_Info where cAccName = '$username' ");
$row = mssql_num_rows($query);
if($row !=0){
	while ($rows = mssql_fetch_assoc($query)) {
		$nExtPoint = $rows['nExtPoint1'];
	}
}

$querysub = mssql_query("select * from Account_sub_Info where cAccName = '$username' ");
$row = mssql_num_rows($querysub);
if($row !=0){
	while ($rows = mssql_fetch_assoc($querysub)) {
		$cPhone = $rows['cPhone'];
		$cEmail = $rows['cEMail'];
		$cRealName = $rows['cRealName'];
    
	}
}
if($_POST['submit']){
  mssql_query("UPDATE Account_sub_Info set cRealName='" . $_POST['hoten'] . "' WHERE cAccName='" . $username . "'");
   $message = "<p class = 'required'><b>Thay đổi thông tin thành công.</b><a href='index.php'> <b><i>Trở lại</i></b></a></p>";
}
    		...
    		<div class="col-sm-3 col-md-8">
				<div class="panel panel-primary">
					<div class="panel-heading">
						<b><span class="fontawesome-calendar" style="float:left;padding-right:10px;"></span>	Cập nhật thông tin tài khoản
						</b>
				...
				<div class="panel-body">
    						...
    						<!--<table class="table">
						        ...
						        <tbody>
						          ...
						          <tr>
						            ...
						            <td><b><span class="fontawesome-reorder"> Họ và tên</span></b></td>
						            <td>
                           <input type="text" name="hoten" value="<?php echo $cRealName; ?>"  >      
                        </td>
						            
						          </tr>
						          <tr>
						            <td><b><span class="fontawesome-envelope"> Email</span></b></td>
						            <td><?php print $cEmail ;?></td>
						          </tr>
						          <tr>
						            <td><b><span class="fontawesome-phone"> Số điện thoại</span></b></td>
						            <td><?php print $cPhone ;?></td>
						          </tr>
						          <tr>
						            <td><b><span class="fontawesome-money"> Số Xu</span></b></td>
						            <td><?php print $nExtPoint*10 ;?></td>
						          </tr>
                      <tr>
                        <td><input type="submit" name="submit" class = "btn btn-primary" value="Cập nhật thông tin" ></td>
                      </tr>
                  ...
                  <form name="frmChange" method="post" action="capnhatthongtin.php?action=edit" ">
